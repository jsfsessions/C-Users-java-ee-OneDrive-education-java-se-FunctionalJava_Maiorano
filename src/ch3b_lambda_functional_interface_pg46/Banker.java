package ch3b_lambda_functional_interface_pg46;

import book.Account;
import ch3a_lambda_pg45.Exemptable;
...
public class Banker {
    ...
    //onAccountExepted from the interface just added AccountExemptionHandler assuming the client's account balance is non sufficient for a withdrawal we want to make an exemption and allow him to withdraw; onAccountExempted if true let him borrow; here we use void instead of boolean as a return 
    public boolean isBalanceSufficient(Account account, double amount, Exemptable ex, AccountExemptionHandler handler) {
        ...
        if (!isBalanceSufficient) {
            if (ex.isExempt(account)) {
                ...
                alertOverdrawn(account);
                // client has an option to withdraw money and incurr fees or chose different
    ...
    public static void main(String[] args) {
        ...
        //start the onAccountExepmted with an anonymous class
        banker.isBalanceSufficient(account, amount, acc -> acc.getBalance() - amount > 700,
            ...
            public void onAccountExempted(Account account) {
                System.out.println("account exemption privilages granted");
            }
        });
